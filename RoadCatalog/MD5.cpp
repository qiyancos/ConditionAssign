...
void MD5::md5_passwd(char *oldpasswd, char *md5_passwd,int size)
{
	md5_state_t state;
	...
	md5_init(&state);
	md5_append(&state, (const md5_byte_t *)oldpasswd, size);
	md5_finish(&state, digest);
#ifdef WIN32
	sprintf(md5_passwd,"\0");
#else
	//snprintf(md5_passwd,sizeof(md5_passwd),"\0");
	sprintf(md5_passwd,"\0");
#endif
	for(di=0; di<16; di++)
#ifdef WIN32
...
#else
		//snprintf(md5_passwd,sizeof(md5_passwd),"%s%02x",md5_passwd,digest[di]);
		sprintf(md5_passwd,"%s%02x",md5_passwd,digest[di]);
#endif
